...
function update_github(){
    local code_dir=${1?}
    repo_list="devops_public code-with-docker chatops_slack cleanup_old_files dennytest detect_suspicious_process developer-free-saas devops_docker_image devops_jenkins directory-cli-tool droplet-neighbor-check elasticsearch-cli-tool images jenkins_image monitor-docker-slack monitoring nmap-scan-docker node_status_in_json python-hosts-tool python-selenium remote-commands-servers shadowsocks-vpn-docker sonarqube-by-docker terraform_jenkins_digitalocean today-learning my-slides challenges-aws-ecs"
